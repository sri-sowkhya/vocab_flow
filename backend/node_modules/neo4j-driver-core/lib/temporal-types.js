...
var Duration = /** @class */ (function () {
    ...
    Duration.fromString = function (str) {
        ...
        if (str.slice(-1).toUpperCase() === 'T') {
            throw (0, error_1.newError)("Duration string '".concat(str, "' ends with 'T', time delimiter must be excluded if Duration contains no time components"));
        }
        var matches = String(str.replace(/,/g, '.')).match(new RegExp(/^([-|+]?)[P|p](?:(-?\d*\.?\d*)[Y|y])?(?:(-?\d*\.?\d*)[M|m])?(?:(-?\d*\.?\d*)[W|w])?(?:(-?\d*\.?\d*)[D|d])?/.source + // Date portion
            /([T|t](?:(-?\d*\.?\d*)[H|h])?(?:(-?\d*\.?\d*)[M|m])?(?:(-)?(\d*)(\.\d*)?[S|s])?)?$/.source // Time portion
        ));
        if (matches === null) {
            throw (0, error_1.newError)("Duration could not be parsed from string: ".concat(str));
        }
        ...
        var negativeDuration = matches[1] === '-' ? -1 : 1;
        var months = negativeDuration * parseTemporalFloat(matches[2], 'Years') * 12 + parseTemporalFloat(matches[3], 'Months');
        var days = negativeDuration * parseTemporalFloat(matches[4], 'Weeks') * 7 + parseTemporalFloat(matches[5], 'Days');
        var hoursAndMinutes = (parseTemporalFloat(matches[7], 'Hours') * 3600 + parseTemporalFloat(matches[8], 'Minutes') * 60);
        var seconds = parseTemporalInt(((_a = matches[9]) !== null && _a !== void 0 ? _a : '') + ((_b = matches[10]) !== null && _b !== void 0 ? _b : '0'));
        var nanos = negativeDuration * parseDurationNanos(((_c = matches[9]) !== null && _c !== void 0 ? _c : '') + '0' + ((_d = matches[11]) !== null && _d !== void 0 ? _d : ''));
        ...
        return new Duration((0, integer_1.int)(months), (0, integer_1.int)(days), (0, integer_1.int)(BigInt(negativeDuration) * (BigInt(hoursAndMinutes) + seconds.toBigInt())), (0, integer_1.int)(nanos));
    };
    /**
     * @ignore
     */
...
var LocalTime = /** @class */ (function () {
     ...
     * @returns {LocalTime}
     */
    LocalTime.fromString = function (str) {
        var values = String(str.replace(/,/g, '.')).match(/^[T|t]?(\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(\.\d+)?$/);
        if (values === null) {
            throw (0, error_1.newError)('LocalTime could not be parsed from string');
        }
        var _a = __read(handleTimeDecimals(values[1], values[2], values[3], values[4]), 4), hours = _a[0], minutes = _a[1], seconds = _a[2], nanoseconds = _a[3];
...
var Time = /** @class */ (function () {
    ...
    Time.fromString = function (str) {
        var values = String(str.replace(/,/g, '.')).match(/^[T|t]?(\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(\.\d+)?([Z|z]$|\+|-)(\d{2})?(?::?(\d{2}))?(?::?(\d{2}))?$/);
        if (values === null) {
            throw (0, error_1.newError)('Time could not be parsed from string');
        }
        var _a = __read(handleTimeDecimals(values[1], values[2], values[3], values[4]), 4), hours = _a[0], minutes = _a[1], seconds = _a[2], nanoseconds = _a[3];
        if (values[5] === 'Z') {
            return new Time(hours, minutes, seconds, nanoseconds, (0, integer_1.int)(0));
        }
        if (values[6] == null) {
            throw (0, error_1.newError)('Error parsing timezone offset.');
        }
        return new Time(hours, minutes, seconds, nanoseconds, (0, integer_1.int)((values[5] === '+' ? 1 : -1) * (parseTemporalInt(values[6]).toInt() * 3600 + // timezone offset hours
...
var Date = /** @class */ (function () {
    ...
    Date.fromString = function (str) {
        var values = String(str.replace(/,/g, '.')).match(/^([+|-]\d{5,}|\d{4})-(\d{2})-(\d{2})$/);
        if (values === null) {
            throw (0, error_1.newError)('Date could not be parsed from string. Expects date in format \'YYYY-MM-DD\' or \'[+|-]YYYYY-MM-DD\'');
        }
...
var LocalDateTime = /** @class */ (function () {
     ...
     */
    LocalDateTime.fromString = function (str) {
        var values = String(str.replace(/,/g, '.')).match(/^([+|-]\d{5,}|\d{4})-(\d{2})-(\d{2})[T|t](\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(\.\d+)?$/);
        if (values === null) {
            throw (0, error_1.newError)('LocalDateTime could not be parsed from string');
        }
        var _a = __read(handleTimeDecimals(values[4], values[5], values[6], values[7]), 4), hours = _a[0], minutes = _a[1], seconds = _a[2], nanoseconds = _a[3];
...
var DateTime = /** @class */ (function () {
     ...
     * @returns {DateTime}
     */
    DateTime.fromString = function (str) {
        var values = String(str.replace(/,/g, '.')).match(new RegExp(/^([+|-]\d{5,}|\d{4})-(\d{2})-(\d{2})[T|t](\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(\.\d+)?/.source + // DateTime
            /([Z|z]|\+|-)?(?:(\d{2})(?::?(\d{2}))?(?::?(\d{2}))?)?(?:\[([^\]]*)\])?$/.source // Timezone
        ));
        if (values === null) {
            throw (0, error_1.newError)('DateTime could not be parsed from string');
        }
        var _a = __read(handleTimeDecimals(values[4], values[5], values[6], values[7]), 4), hours = _a[0], minutes = _a[1], seconds = _a[2], nanoseconds = _a[3];
        if (values[8] === 'Z') {
            if (values[9] != null) {
                throw (0, error_1.newError)('DateTime could not be parsed from string, can not parse string with offset after Z');
            }
            return new DateTime(parseTemporalInt(values[1]), // years
            parseTemporalInt(values[2]), // months
            parseTemporalInt(values[3]), // days
            hours, minutes, seconds, nanoseconds, (0, integer_1.int)(0), values[12]);
        }
        if (values[8] === '+' || values[8] === '-') {
            if (values[9] == null) {
                throw (0, error_1.newError)("DateTime could not be parsed from string, could not parse an offset after ".concat(values[8], " sign."));
            }
            return new DateTime(parseTemporalInt(values[1]), // years
            parseTemporalInt(values[2]), // months
            parseTemporalInt(values[3]), // days
            hours, minutes, seconds, nanoseconds, (0, integer_1.int)((values[8] === '+' ? 1 : -1) * (parseInt(values[9]) * 3600 + parseInt(values[10]) * 60 + parseInt('0' + values[11]))), values[12]);
        }
        ...
        throw (0, error_1.newError)('DateTime could not be parsed from string, provided string needs either timezoneId or offset');
    };
    /**
     * @private
     * @returns {number}
...
function parseTemporalFloat(str, field) {
    if (str === undefined || str.length === 0) {
        ...
        if (isNaN(value)) {
            throw (0, error_1.newError)("Failed to parse temporal field ".concat(field, ". Got string: ").concat(str, "."));
        }
        return value;
    }
}
function parseTemporalInt(str) {
    if (str === undefined || str.length === 0) {
        return (0, integer_1.int)(0);
    }
    return (0, integer_1.int)(str);
}
function handleTimeDecimals(hourString, minuteString, secondString, decimalString) {
    ...
    var seconds;
    var nanoseconds;
    var decimalNumber = decimalString !== undefined ? parseFloat('0' + decimalString) : 0;
    if (minuteString === undefined || minuteString === '') {
        hours = parseTemporalInt(hourString);
        minutes = (0, integer_1.int)(decimalNumber * 60);
        seconds = (0, integer_1.int)((decimalNumber * 3600) % 60);
        nanoseconds = (0, integer_1.int)((decimalNumber * 3600 * Math.pow(10, 9)) % Math.pow(10, 9));
    }
    else if (secondString === undefined || secondString === '') {
        hours = parseTemporalInt(hourString);
        minutes = parseTemporalInt(minuteString);
        seconds = (0, integer_1.int)((decimalNumber * 60) % 60);
        nanoseconds = (0, integer_1.int)((decimalNumber * 60 * (Math.pow(10, 9))) % Math.pow(10, 9));
    }
    else {
        hours = parseTemporalInt(hourString);
        minutes = parseTemporalInt(minuteString);
        seconds = parseTemporalInt(secondString);
        nanoseconds = (0, integer_1.int)(decimalNumber * Math.pow(10, 9));
    }
...
function checkDurationFieldIsInt(value, field) {
    if (!Number.isInteger(value)) {
        throw (0, error_1.newError)("Parsing Duration field: ".concat(field, " resulted in a non-integer value. Bolt protocol can only send durations which can be simplified to integer values of months, days, seconds and nanoseconds."));
    }
}
